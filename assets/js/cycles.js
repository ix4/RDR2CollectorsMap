var Cycles = {
  ...
  checkForUpdate: function () {
    ...
    if (day != Cycles.setLocaleDate()) { $('.map-cycle-alert').removeClass('hidden') } else { $('.map-cycle-alert').addClass('hidden') }
  },
    ...
    $.each(Cycles.data.cycles, function (key, value) {
      ...
      $.each(value, function (_k, _c) {
        if (_k == 'card_pentacles' || _k == 'card_swords' || _k == 'card_wands' ||
                    _k == 'lost_bracelet' || _k == 'lost_earrings' || _k == 'lost_necklaces') { return }
