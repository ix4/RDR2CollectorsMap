...
var MapBase = {
  ...
  setMarkers: function (data) {
    ...
    if (date != $.cookie('date') && Settings.resetMarkersDaily) {
      ...
      $.each(markers, function (key, value) {
        ...
        if (Inventory.isEnabled) { markers[key].canCollect = value.amount < Inventory.stackSize } else { markers[key].canCollect = true }
      })
  ...
  onSearch: function (searchString) {
    ...
    $.each(searchString.split(';'), function (key, value) {
      if ($.inArray(value.trim(), searchTerms) == -1) {
        if (value.length > 0) { searchTerms.push(value.trim()) }
      }
    })
    ...
    if (searchTerms.length == 0) {
      ...
      $.each(searchTerms, function (id, term) {
        searchMarkers = searchMarkers.concat(MapBase.markers.filter(function (_marker) {
          if (_marker.title != null) { return _marker.title.toLowerCase().includes(term.toLowerCase()) }
        }))
  ...
  addMarkers: function (refreshMenu = false) {
    if (Layers.itemMarkersLayer != null) { Layers.itemMarkersLayer.clearLayers() }
    if (Layers.miscLayer != null) { Layers.miscLayer.clearLayers() }
    ...
    $.each(MapBase.markers, function (key, marker) {
      ...
      if (marker.subdata != null) {
        if (categoriesDisabledByDefault.includes(marker.subdata)) { return }
      }
    ...
    if (refreshMenu) { Menu.refreshMenu() } else {
      Routes.generatePath()
      return
    }

    if (Routes.generateOnVisit) { Routes.generatePath(true) }
  ...
  removeItemFromMap: function (day, text, subdata, category, skipInventory = false) {
    if (text.endsWith('_treasure')) {
      ...
      } else { Treasures.enabledTreasures.push(text) }
    ...
    if (Routes.ignoreCollected) { Routes.generatePath() }
  ...
  addMarkerOnMap: function (marker, opacity = 1) {
    ...
    if (marker.category == 'random') { marker.title = `${Language.get('random_item.name')} #${marker.text.split('_').pop()}` } else if (marker.category == 'american_flowers') { marker.title = `${Language.get(`flower_${marker.subdata}.name`)} #${marker.text.split('_').pop()}` } else if (marker.category == 'bird_eggs' && (marker.subdata == 'eagle' || marker.subdata == 'hawk')) { marker.title = `${Language.get(`egg_${marker.subdata}.name`)} #${marker.text.split('_').pop()}` } else { marker.title = Language.get(`${marker.text}.name`) }

    if (marker.subdata == 'agarita' || marker.subdata == 'blood_flower') { marker.description = Language.get(`${marker.text}_${marker.day}.desc`) + ' ' + Language.get('map.flower_type.night_only') } else if (marker.subdata == 'creek_plum') { marker.description = Language.get(`${marker.text}_${marker.day}.desc`) + ' ' + Language.get('map.flower_type.bush') } else if (marker.subdata == 'spoonbill' || marker.subdata == 'heron' || marker.subdata == 'eagle' || marker.subdata == 'hawk' || marker.subdata == 'egret') { marker.description = Language.get(`${marker.text}_${marker.day}.desc`) + ' ' + Language.get('map.egg_type.tree') } else if (marker.subdata == 'vulture') { marker.description = Language.get(`${marker.text}_${marker.day}.desc`) + ' ' + Language.get('map.egg_type.stump') } else if (marker.subdata == 'duck' || marker.subdata == 'goose' || marker.subdata == 'loon') { marker.description = Language.get(`${marker.text}_${marker.day}.desc`) + ' ' + Language.get('map.egg_type.ground') } else { marker.description = Language.get(`${marker.text}_${marker.day}.desc`) }
    ...
    if (Settings.markerCluster) { Layers.oms.addMarker(tempMarker) }
  ...
  highlightImportantItem (text, category) {
    if (category === 'american_flowers' || category === 'bird_eggs') { text = text.replace(/(egg_|flower_)(\w+)(_\d)/, '$2') }
    ...
    if ($(`[data-marker*=${text}].highlight-items`).length) { MapBase.itemsMarkedAsImportant.push(text) } else { MapBase.itemsMarkedAsImportant.splice(MapBase.itemsMarkedAsImportant.indexOf(text), 1) }
  ...
  loadImportantItems () {
    if (typeof localStorage.importantItems === 'undefined') { localStorage.importantItems = '[]' }
  ...
  submitDebugForm: function () {
    ...
    if (!isNaN(lat) && !isNaN(lng)) { MapBase.debugMarker(lat, lng) }
  },
  ...
  addCoordsOnMap: function (coords) {
    ...
    if (Settings.isPinsPlacingEnabled) { Pins.addPin(coords.latlng.lat, coords.latlng.lng) }
  },
