...
var Menu = {
  ...
  refreshTreasures: function () {
    ...
    Treasures.data.filter(function (item) {
      ...
      if (!Treasures.enabledTreasures.includes(item.text)) { collectibleElement.addClass('disabled') }
...
Menu.refreshMenu = function () {
  ...
  $.each(MapBase.markers, function (_key, marker) {
    ...
    if (marker.category != 'random') { collectibleImage = $('<img>').attr('src', `./assets/images/icons/game/${collectibleKey}.png`).addClass('collectible-icon') }
    ...
    collectibleElement.on('contextmenu', function (event) {
      ...
      if (marker.subdata != 'agarita' && marker.subdata != 'blood_flower') { MapBase.highlightImportantItem(marker.subdata || marker.text) }
    })
    ...
    if (!Inventory.isEnabled) { collectibleCountElement.hide() }

    var collectibleCategory = $(`.menu-option[data-type=${marker.category}]`)
    if (marker.lat.length == 0 || marker.tool == -1) {
      if (!anyUnavailableCategories.includes(marker.category)) { anyUnavailableCategories.push(marker.category) }
    ...
    if (collectibleCategory.hasClass('not-found') && !anyUnavailableCategories.includes(marker.category)) { collectibleCategory.attr('data-help', 'item_category').removeClass('not-found') }

    if (Inventory.isEnabled && marker.amount >= Inventory.stackSize) { collectibleElement.addClass('disabled') }

    if (marker.subdata) {
      if (marker.subdata == 'agarita' || marker.subdata == 'blood_flower') { collectibleElement.attr('data-help', 'item_night_only') }

      var currentSubdataMarkers = MapBase.markers.filter(function (_marker) {
        if (marker.subdata != _marker.subdata) { return false }

        if (_marker.day != Cycles.data.cycles[Cycles.data.current][_marker.category]) { return false }

        return true
      })

      if (currentSubdataMarkers.every(function (marker) { return !marker.canCollect })) { collectibleElement.addClass('disabled') }
    } else {
