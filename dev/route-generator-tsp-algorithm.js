...
function Path (points) {
  ...
  for (var i = 0; i < points.length; i++) {
    for (var j = 0; j < points.length; j++) { this.distances[j + i * points.length] = distance(points[i], points[j]) }
  }
};

Path.prototype.change = function (temp) {
  var i = this.randomPos(); var j = this.randomPos()
...
Path.prototype.delta_distance = function (i, j) {
  ...
  var jp1 = this.index(j + 1)
  var im1 = this.index(i - 1)
  var ip1 = this.index(i + 1)
  var s =
        this.distance(jm1, i) +
        this.distance(i, jp1) +
        this.distance(im1, j) +
